...
if ($class_id === 0 || $receiver_id === 0) {
    die("Invalid request.");
}

// Mark messages as seen when user opens chat
$update_stmt = $conn->prepare("UPDATE messages SET seen = 1 WHERE class_id = ? AND receiver_id = ?");
$update_stmt->bind_param("ii", $class_id, $sender_id);
$update_stmt->execute();
$update_stmt->close();
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["message"])) {
    ...
    if (!empty($message)) {
        $stmt = $conn->prepare("INSERT INTO messages (class_id, sender_id, receiver_id, message, seen) VALUES (?, ?, ?, ?, 0)");
...
<head>
    ...
    <title>Message <?php echo htmlspecialchars($receiver['name']); ?></title>
    <style>
        body {
...
function checkNewMessages() {
    document.querySelectorAll(".message-button").forEach(button => {
        ...
        fetch(`check_unread_messages.php?class_id=${classId}`)
...
// Auto-refresh unread message status every 5 seconds
